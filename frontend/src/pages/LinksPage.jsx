...
const LinksPage = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#b0bb9c]">
      <div className="bg-[#fffbe6] shadow-lg p-0 w-full max-w-md flex flex-col items-center" style={{ borderRadius: 0 }}>
        ...
        <div className="w-full flex justify-between items-center px-6 py-4">
          ...
          >
            ...
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-[#678965]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 8a3 3 0 00-6 0v8a3 3 0 006 0V8z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 8v8a7 7 0 01-14 0V8" />
            </svg>
            {copiedId === 'page' && (
        ...
        {/* Username */}
        <h2 className="text-2xl font-bold text-gray-800 mb-1 text-center w-full">{userData.username}</h2>
        {/* Title/Description */}
        <p className="text-gray-600 mb-6 text-center w-full">{userData.title || userData.description}</p>
     ...
     <div className="w-full flex flex-col gap-4 px-6 pb-8">
          {userData.links.map((url) => (
            <div
              ...
              onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 0 3px #b0bb9c'}
              onMouseLeave={e => e.currentTarget.style.boxShadow = '0 1px 4px 0 rgba(104,137,101,0.10)'}
            >
              <a
                href={url.url}
                target="_blank"
              ...
              <button
                onClick={e => { e.stopPropagation(); handleCopy(url.url, url._id); }}
